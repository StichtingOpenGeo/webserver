 ...
 *
 * Copyright (C) 2001-2012 Alvaro Lopez Ortega
 *
			     ...
			     cherokee_module_props_t *props)
{
	...
	n->socket = zmq_socket(PROP_TMI(props)->context, ZMQ_PUSH);
	if (zmq_connect (n->socket, PROP_TMI(props)->endpoint.buf) != 0) {
		TRACE(ENTRIES, "ZeroMQ Connect: can't connect to '%s'\n", PROP_TMI(props)->endpoint.buf);
		return ret_error;
	}
	n->encoder = NULL;
